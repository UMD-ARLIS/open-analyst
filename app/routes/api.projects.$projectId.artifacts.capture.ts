...
async function extractTextFromBuffer(
  ...
  if (type.includes('wordprocessingml.document') || lowerName.endsWith('.docx')) {
    try {
...
export async function action({ request, params }: Route.ActionArgs) {
  ...
  const extension =
    path.extname(requestedFilename || workspacePath) || inferExtension(String(body.mimeType || ''));
  ...
  const artifactRecord = await createArtifact(projectId, {
    title: String(body.title || '').trim() || path.basename(requestedFilename || workspacePath),
  ...
  const version = await createArtifactVersion(projectId, artifactRecord.id, {
    ...
    changeSummary: String(body.changeSummary || '').trim() || 'Initial captured version',
    storageUri: artifact.storageUri,
    contentText: content,
    metadata: {
      filename: artifact.filename,
